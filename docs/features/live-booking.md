...
**Not yet run on a deployment.** The dev deployment rejects every schema push right now because it holds rows with fields no branch declares (`projects.body`, `storyUpdates.bodyDoc`) — a pre-existing problem, filed as bead wonderwall-vx7f. Follow-ups below are bead wonderwall-96j0. Until that is cleared, this feature has unit tests and a type check behind it, not a click-through. One thing to confirm on the first push: the time-zone math uses `Intl.DateTimeFormat` inside Convex mutations. It works in Node (the tests), and Convex's runtime is V8 with the same web globals, but it hasn't been exercised there yet. If it ever fails, the fallback is small: compute each date's start and end in the browser and pass them in, and have the nightly job reuse the last known offset.
