...
- `table.create` — create a **Table**: a *continuing* gathering with a roster — people belong, it has a term, it can spawn a cohort, it gets a curated project space and patron routing. The event/table line: attendees show up; a roster belongs. Not a tenant — host *organizations* are provisioned by us, not self-serve
  ...
  **Nav buckets teach themselves (decided 2026-08-12):** the browse sections are mutually exclusive along one axis — you *attend* an Event, you *belong to* a Table, you *take someone up on* an Offer — and every section carries a one-line descriptor stating it, with Events/Tables cross-linking each other ("ongoing classes live under Tables" / "one-off workshops live under Events"), since a workshop can be either depending on its time-shape. Every Offer card states free or priced; every priced Event's CTA carries its price. Mocked at /demo/app.
