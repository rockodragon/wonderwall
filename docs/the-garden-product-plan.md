...
- `table.create` — create a **Table**: a *continuing* gathering with a roster — people belong, it has a term, it can spawn a cohort, it gets a curated project space and patron routing. The event/table line: attendees show up; a roster belongs. Not a tenant — host *organizations* are provisioned by us, not self-serve
  ...
  **Classes, mentorship, coaching (DECIDED 2026-08-10, prompted by AP's "Pathfinding"):** primitives cut by social shape; content labels are a **format tag** on the card (Workshop, Class, Mentorship, Critique, Show…). One-off workshop = Event; multi-week class = cohort Table; group mentorship = member/cohort Table; **1:1 coaching = an Offer** (partner-light offers extended from places to people — coaching is hiring, not belonging; a table-of-two was considered and rejected because Tables carry community machinery). "Pathfinding" is AP's program brand, shown as a byline on their cards — never platform nav. No new nav item; a host's page is their "classes & coaching" shelf; a cross-cutting "Learn" filter only if demand shows. Tracked: wonderwall-nk7.

  **Nav buckets teach themselves (decided 2026-08-12, revised same day):** the browse sections are mutually exclusive along one axis — you *attend* an Event, you *belong to* a Table, you *take someone up on* an Offer. Every section carries a one-line, benefit-forward descriptor (no mid-sentence cross-references — Rick: they read as excuses; the register to match is Projects' "Creative work seeking support, and paid work seeking creatives"). Findability across the Events/Tables boundary is carried by format tags + per-section format filters, not by copy. Every browse section has minimum interactivity: format filter chips + a gated add-action ("+ Start a project" etc.) whose denial renders the can() reason and upgrade path. Every Offer card states free or priced; every priced Event's CTA carries its price. Mocked at /demo/app.

  **Discovery + nav (decided 2026-08-10):** coaching is found in three places — the **Offers directory** (the partner-light directory, extended to person-offers; primary), the coach's profile, and their Table page. No separate coaching directory. Post-Phase-2 nav: **Projects · Events · Tables · Offers** — "Tables" is a nav item; nav is where the vocabulary gets taught (cf. Discord "Servers", Skool "Classrooms"), and the card design explains it on first click. Mocked live at `/demo/offers`.
...
**Status 2026-08-09:** Phase 0 largely shipped — `/` prerendered (title, meta, OG, hero without JS), `/about/*` rebranded to the credit-sheet system with all four personas + partners, `/about/leaders` 301s to `/about/hosts`, and the first Garden event is live (unlinked) at `/events/first-table/` with safe placeholders. Full mock set for Phases 0–3 lives in `docs/mocks/` (creative flow, project gates, patron doors, host tables, host dashboard, partners light, story page, Buzz, first event). Still open in Phase 0: YouTube channel link, public story pages. Follow-ups tracked in beads.

**Status 2026-08-09 (later):** the app-UI vocabulary pass shipped (Jobs→Projects in all visible copy; URLs and identifiers unchanged, tracked for Phase 1B). **The Phase 1 demo build shipped at `/demo`** (noindex, unlinked): the four mocked flows as real screens — join (make→fund→place→gather → tier → checkout-ends-in-invitation), create (passion/paid gates with full denial anatomy), patron (two doors + GRACE-FALL coverage codes + redemption), host (wizard with product-enforced free-door rule + dashboard: Today/People-with-provenance/Attendance/Money-ledger→$324/Grow) — driven by a persona switcher (Maya/Shua/Tessa/Marcus/Diane/Folded Note) and a **real `can(user, capability)` service implementing §2.3 verbatim** (`app/app/garden/capabilities.ts`). Phase 1B swaps Stripe + Convex in behind that contract; the UI callers don't change. All `/demo/*` routes are prerendered — required, because the SPA fallback serves the home document and its hydration mismatch kills interactivity on deep links.
